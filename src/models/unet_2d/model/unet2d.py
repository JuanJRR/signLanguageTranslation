...
class Unet2D(nn.Module):
    def __init__(
        ...
        # encoder
        self.encoder_1 = DownConv2D(in_channels=in_channels, out_channels=channels)
        self.encoder_2 = DownConv2D(in_channels=in_channels, out_channels=channels)
        self.encoder_3 = DownConv2D(in_channels=in_channels, out_channels=channels)

        # bottleneck
        self.bottleneck = nn.Sequential(
            nn.MaxPool2d(kernel_size=2, stride=2),
            Conv2D(in_channels=in_channels, out_channels=channels),
        )

        # decoder
        self.decoder_3 = UpConv2D(in_channels=in_channels, out_channels=channels)
        self.decoder_2 = UpConv2D(in_channels=in_channels, out_channels=channels)
        self.decoder_1 = UpConv2D(in_channels=in_channels, out_channels=channels)
        self.decoder_input = UpConv2D(in_channels=in_channels, out_channels=channels)
